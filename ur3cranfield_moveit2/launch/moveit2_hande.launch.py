...
def generate_launch_description():
    ...
    # === ROBOTIQ HAND-E CONTROLLER === #
    hande_controller_spawner_LEFT = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["hande_controller_LEFT", "-c", "/controller_manager"],
    )
    hande_controller_spawner_RIGHT = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["hande_controller_RIGHT", "-c", "/controller_manager"],
    )
    ...
    moveit_simple_controllers_yaml = load_yaml("ur3cranfield_moveit2", "config/ur_controller_moveit2.yaml")
    moveit_controllers = {
    ...
    MoveInterface = Node(
        ...
        executable="move",
        output="screen",
        parameters=[robot_description, robot_description_semantic, kinematics_yaml, {"use_sim_time": True}, {"ROB_PARAM": "ur3"}, {"EE_PARAM": "robotiq_hande"}, {"ENV_PARAM": "gazebo"}],
    )
        ...
        [
            ...
            RegisterEventHandler(
                OnProcessExit(
                    ...
                    on_exit = [
                        hande_controller_spawner_LEFT,
                        hande_controller_spawner_RIGHT,
                    ]
                )
            ),
            ...
            RegisterEventHandler(
                OnProcessExit(
                    target_action = hande_controller_spawner_RIGHT,
                    on_exit = [
