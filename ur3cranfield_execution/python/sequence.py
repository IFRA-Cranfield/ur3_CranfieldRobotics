...
class CloseProgram():
    def CLOSE():
        ...
        print('COMMAND -> ros2 run ur3cranfield_execution sequence.py --ros-args -p PROGRAM_FILENAME:="---" -p ROBOT_MODEL:="---" -p EE_MODEL:="---" -p GzBr_ENV:="---"')
        print("Closing... BYE!")
        time.sleep(5)
        exit()
...
def main(args=None):
    ...
    print("ur3cranfield_execution --> SEQUENCE EXECUTION")
    ...
    filepath = os.path.join(os.path.expanduser('~'), 'dev_ws', 'src', 'ur3_CranfieldRobotics', 'ur3cranfield_execution', 'programs', PR_NAME + ".txt")
    ...
    for i in range (1, len(readSEQ)+1):
        ...
        elif (ACTION.action == "RobotiqHandE - GripperOpen"):
            pass # No action required, since ACTION name is passed and enough.
        elif (ACTION.action == "RobotiqHandE - GripperClose"):
            pass # No action required, since ACTION name is passed and enough.
